...
public class Activitycdc extends AppCompatActivity {
    ...
    class Connector extends AsyncTask<String, String, String> {
        ...
        protected String doInBackground(String... params) {
            ...
            String link = "http://10.216.2.136/android_connect/cdc.php"; //10.216.2.136  10.236.39.36 10.236.46.44 holds the link for the php file I want to connect to
            try{
                ...
                String line = ""; //will store a line of the read contents of phpread
                while ((line = phpread.readLine()) != null) { //loop reads through each line of the php file, and adds the contents to the stringBuffer
            ...
            final String res_fin4 = dataread; //result
            runOnUiThread(new Runnable() { //runONUi thread allows content to run on main thread since, only there can necessary code be run
                @Override
                public void run() {
                    setContentView(R.layout.activity_activitycdc ); //adds desired layout to be displayed
                    TextView textView2 = (TextView)findViewById(R.id.textViewcdc); //connects a text variable to text button in the layout
                    textView2.setText(res_fin4); //sets a value to the text
